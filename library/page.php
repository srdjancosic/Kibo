...
class Page extends Functions {
	...
	function getTitle($pageURL) {
		$query = Database::execQuery("SELECT * FROM pages WHERE url = '$pageURL'".$this->langSQL);
	...
	function getDescription($pageURL) {
		...
		if(mysql_num_rows($query) == 1) {
			...
			if($data['page_description'] != "") { // if there is defined title, then show it
				...
				if($nodeDescription != "") echo stripslashes(strip_tags($nodeDescription)) . " | ";
				
				$categoryDescription = Database::getValue("category_description", "category", "id", $this->categoryId);
				if($categoryDescription != "") echo stripslashes(strip_tags($categoryDescription)) . " | ";
			}
		}
	}
	...
	function getKeywords($pageURL) {
		...
		if(mysql_num_rows($query) == 1) {
			...
			if($data['page_keywords'] != "") { // if there is defined title, then show it
				...
				if($nodeKeywords != "") echo stripslashes(strip_tags($nodeKeywords)) . ", ";
				
				$categoryKeywords = Database::getValue("category_keywords", "category", "id", $this->categoryId);
				if($categoryKeywords != "") echo stripslashes(strip_tags($categoryKeywords)) . ", ";
			}
		}
	}
	...
	function printLeafContent($content_type, $content, $css_class, $css_id) {
			...
			switch ($content_type) {
				...
				case "menu":
					...
					//list($c_name, $c_css, $c_content) = explode("|:|", $content);
					$c_content = $content;
					//$css_ = ($c_css == "") ? $css_class : $c_css;
					...
					foreach($menu_items as $key2 => $menu_item) {
						...
						foreach($menu_item as $key => $value) {
							...
							if($item_type == "category") {
								...
								/*
								$subquery = Database::execQuery("SELECT * FROM category WHERE parent = '$item_id'");
								$parent_link = $item_link;
								$submenu = "";
								if(mysql_num_rows($subquery) > 0) {
									$submenu .= "<ul>";
									while ($subdata = mysql_fetch_array($subquery, MYSQL_ASSOC)) {
										$subURL = ($subdata['href'] == "") ? $subdata['url'] : $subdata['href'];
										$submenu .= "<li><a href=\"/".$this->lang_code."/".$subURL."\">".$subdata['name']."</a></li>";
									}
									$submenu .= "</ul>";
								}*/
				...
				case "slider":
					...
					$output .= "<script type='text/javascript' src='/modules/aviaslider/js/jquery.aviaSlider.min.js'></script>";
					$output .= "<link rel=\"stylesheet\" href=\"/modules/aviaslider/css/style.css\" type=\"text/css\" media=\"screen\" />";
					...
					$output .= "<ul class='aviaslider' id=\"frontpage-slider\">";
						$query = Database::execQuery("SELECT * FROM file_manager WHERE category = '$c_album'");
						while ($data = mysql_fetch_array($query, MYSQL_ASSOC)) {
							$output .= "<li><img src=\"".$data['file_path'].$data['file_name']."\" alt=\"".$data['name']."\" /></li>";
						}
					$output .= "</ul>";
					$output .= "</div>";
					
					/*
					$output .= "$('#frontpage-slider').aviaSlider({	blockSize: {height: 80, width:80},
						transition: 'slide',
						display: 'all',
					...
					*/
					...
					$output .= "$('#frontpage-slider').aviaSlider();";
					$output .= "</script>";
				...
				case "plugin":
					...
					$plugin->setOptions($this->pluginOpts);
					...
					if(count($folder_list) > 0) {
						...
						if(count($new_path) > 0) {
							...
							foreach($new_path as $id => $value) {
								$file_path[$value] = $_base."/".$_folder."/".$value."/"; //str_replace($base, $new_path, $data['picture']);
							}
				...
				case "form":
						...
						switch($field->field_type) {
							...
							case "select":
								...
								if($field->from_table == 1) {
									...
									$query = mysql_query("SELECT $key, $value FROM $table");
									while($data = mysql_fetch_array($query, MYSQL_ASSOC)) {
							...
							case "radiobutton":
								$checked = ($field->value == $field->selected_value) ? "checked=\"checked\"" : "";
								$output .= "<input type=\"radio\" name=\"".$field->name."\" ".
																 "id=\"$field->identificator\" ".
																 "class=\"$field->class\" ".
																 "value=\"$field->value\" ".
																 "$checked ".
					...
					if(count($validate_fields) > 0) {
						...
						foreach($validate_fields as $identificators => $regExp) {
							...
							$query = mysql_query("SELECT error_message FROM form_fields WHERE identificator = '$identificators' AND form_id = '$form->id'");
							$data = mysql_fetch_array($query, MYSQL_ASSOC);
							...
							$output .= "if(!result_$i) { ".
											"if($('.error_$identificators').length == 0) {".
												"$('#$identificators').parent().append(\"<span class='error error_$identificators'>$error_message</span>\"); ".
											"}".
											
											" return false; }".
										" else { $('#$identificators').parent().find('span.error').each(function() { $(this).remove(); }); }";
	...
	function compile($kml, $data) {
		...
		for($i=0; $i<count($matches[0]); $i++) {
			...
			if($match == "url") {
				...
				$value = "/".$lang_code."/".$category_url."/".$value;
			}
			...
			if($prematch == "picture") {
				...
				if($new_path != "") {
					...
					$tmp_path = "/".$_base."/".$_folder."/".$new_path."/".$_file; //str_replace($base, $new_path, $data['picture']);
					$value = $tmp_path; 
				}
